...
export const seedDatabase = async () => {
  try {
    ...
    const affiliatePasswordHash = await bcrypt.hash('password123', 12);
    
    // Level 1 affiliate (no referrer)
    const level1Result = await db.run(`
      INSERT OR IGNORE INTO users (email, password_hash, name, role, referral_code)
      VALUES (?, ?, ?, ?, ?)
    `, ['john@example.com', affiliatePasswordHash, 'John Smith', 'affiliate', 'JOHN2024']);
    
    const level1User = await db.get('SELECT id FROM users WHERE email = ?', ['john@example.com']);
    ...
    const level2User = await db.get('SELECT id FROM users WHERE email = ?', ['sarah@example.com']);
    ...
    const level3User = await db.get('SELECT id FROM users WHERE email = ?', ['mike@example.com']);
    ...
    if (level1Id) {
      const linkResult = await db.get('SELECT id FROM affiliate_links WHERE affiliate_id = ? LIMIT 1', [level1Id]);
      ...
      if (linkId) {
        ...
        await db.run('UPDATE affiliate_links SET clicks = 25, conversions = 5 WHERE id = ?', [linkId]);
      }
    }
