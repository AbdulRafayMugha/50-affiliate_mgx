...
export const seedDatabase = async () => {
  ...
  try {
    ...
    const { rows: level1Rows } = await client.query(
      `INSERT INTO users (email, password_hash, name, role, referral_code)
       VALUES ('john@example.com', $1, 'John Smith', 'affiliate', 'JOHN2024')
       ON CONFLICT (email) DO NOTHING
       RETURNING id`,
      [affiliatePasswordHash]
    );
    ...
    const { rows: level2Rows } = await client.query(
      `INSERT INTO users (email, password_hash, name, role, referrer_id, referral_code)
       ...
       ON CONFLICT (email) DO NOTHING
       RETURNING id`,
      [affiliatePasswordHash, level1Id]
    );
    ...
    const { rows: level3Rows } = await client.query(
      `INSERT INTO users (email, password_hash, name, role, referrer_id, referral_code)
       ...
       ON CONFLICT (email) DO NOTHING
       RETURNING id`,
      [affiliatePasswordHash, level2Id]
    );
    ...
    if (level1Id) {
      ...
      const { rows: linkRows } = await client.query(
        'SELECT id FROM affiliate_links WHERE affiliate_id = $1 LIMIT 1',
        [level1Id]
      );
      ...
      if (linkRows.length > 0) {
        ...
        for (let i = 1; i <= 5; i++) {
          const { rows: transactionRows } = await client.query(
            `INSERT INTO transactions (customer_email, amount, affiliate_link_id, referrer_id, status)
        ...
        await client.query(
          'UPDATE affiliate_links SET clicks = 25, conversions = 5 WHERE id = $1',
          [linkId]
        );
      }
    }
    ...
    if (level1Id) {
      await client.query(
        `INSERT INTO email_invites (affiliate_id, email, message, status)
         VALUES 
