...
export class EmailService {
  ...
  constructor() {
    ...
    // Accept "true"/"false" (string) or boolean; default false (STARTTLS on 587)
    const secureEnv = process.env.SMTP_SECURE;
    const secure =
      secureEnv === 'true' || secureEnv === '1' || secureEnv === 'TRUE'
        ? true
        : false;
//  || secureEnv === true
    // Auth
    ...
    // For local/dev convenience, allow skipping strict cert checks (DO NOT use in production)
    ...
    const transportOptions: any = {
      ...
      // Only set tls.rejectUnauthorized to false in non-production for self-signed cert dev environments.
      tls: {
        rejectUnauthorized: isProd ? true : false
      }
    };

    // Add debugging/logging in non-production to help diagnose TLS handshakes
    if (!isProd) {
    ...
    // Optional: catch transporter-level errors early and log
    this.transporter.verify().then(
      () => {
        if (!isProd) console.log('EmailService: SMTP transporter verified successfully');
      },
      (err) => {
        // Don't throw here — calling code uses testConnection() and continues on failure
        console.warn('EmailService: SMTP transporter verify failed (constructor):', err && err.message ? err.message : err);
      }
    );
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      // Provide a clear, short message for logs — nodemailer prints long stack; keep that for deeper debugging
