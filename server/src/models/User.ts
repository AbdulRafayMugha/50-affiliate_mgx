...
export class UserModel {
  ...
  static async getAffiliatesByCoordinator(coordinatorId: string, page: number = 1, limit: number = 20): Promise<{
    ...
    const [affiliatesResult, countResult] = await Promise.all([
      ...
      pool.query('SELECT COUNT(*) FROM users WHERE role = \'affiliate\' AND coordinator_id = $1', [coordinatorId])
    ]);
  ...
  static async assignAffiliateToCoordinator(affiliateId: string, coordinatorId: string): Promise<void> {
    await pool.query(
      'UPDATE users SET coordinator_id = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 AND role = \'affiliate\'',
      [coordinatorId, affiliateId]
    );
  }

  static async removeAffiliateFromCoordinator(affiliateId: string): Promise<void> {
    await pool.query(
      'UPDATE users SET coordinator_id = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND role = \'affiliate\'',
      [affiliateId]
    );
  }
